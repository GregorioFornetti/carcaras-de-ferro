/*
    Exemplo de definição de inimigo
    Cria uma classe que herda a classe Enemy que define sua inicialização, update e destruição
    A inicialização basicmanete coloca o inimigo no jogo, enquanto o update realmente define o comportamento do inimigo
    Para gerar um inimigo, siga os passos:
    1) Crie uma classe que herda Schaema.Schema. Esse é o estado sincronizado do inimigo
    2) Defina os atributos com schema.defineTypes
    3) Crie a classe do inimigo que herda Enemy e implemente seu comportamento com update
    4) Pode ser necessário criar uma função SpawnEnemyX que cria uma instância do inimigo "Schema"
*/
import { Enemy } from './Enemy.js';
import * as schema from "@colyseus/schema";


export class EnemyRedSquareSchema extends schema.Schema {
    
}

schema.defineTypes(EnemyRedSquareSchema, {
    x: "number",
    y: "number",
    color: "number"
});

export class EnemyRedSquare extends Enemy {

    static spawn(roomState) {
        const enemy1 = new EnemyRedSquare(roomState);
        enemy1.enemyAttributes.x = 100;
        enemy1.enemyAttributes.y = 100;

        const enemy2 = new EnemyRedSquare(roomState);
        enemy2.enemyAttributes.x = 100;
        enemy2.enemyAttributes.y = 200;

        const enemy3 = new EnemyRedSquare(roomState);
        enemy3.enemyAttributes.x = 200;
        enemy3.enemyAttributes.y = 100;

        const enemy4 = new EnemyRedSquare(roomState);
        enemy4.enemyAttributes.x = 200;
        enemy4.enemyAttributes.y = 200;

        return [enemy1, enemy2, enemy3, enemy4];
    }

    constructor(roomState) {
        super()
        this.init(roomState.enemiesRedSquareSchema, EnemyRedSquareSchema)

        this.enemyAttributes.color = 0;

        this.timeToChangeColor = 5;  // Tempo em segundos para mudar a cor em segundos
        this.currentColor = 0;
    }

    update(deltaTime) {
        this.currentColor += 255 * (deltaTime / 1000) / this.timeToChangeColor;
        this.enemyAttributes.color = Math.min(Math.floor(this.currentColor), 255);

        if (this.currentColor > 255) {
            this.destroy()
        }
    }
}