...
export class Collisor {
    ...
    registerForCollission(gameObj, schema, objtype = "") {
        if (schema.x === undefined || schema.y === undefined) {
            return
        }
        else if(gameObj.width === undefined || gameObj.height === undefined) {
            return
        }
    ...
    update() {
        for (let cobj in this.collisionObjs) {
            this.collisionObjs[cobj].setPosition(
                this.collisionObjs[cobj].schema.x - (this.collisionObjs[cobj].width/2),
                this.collisionObjs[cobj].schema.y - (this.collisionObjs[cobj].height/2))
        }
